...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    // Angle argument expressed in (-IPI/2 .. +IPI/2) as it is easier to think about for user.
    const bool flip_label = (angle < 0.0f);
    angle -= IPI * 0.5f;
    const float cos_a = ImCos(angle);
    const float sin_a = ImSin(angle);
    const float label_cos_a = flip_label ? ImCos(angle + IPI) : cos_a;
    const float label_sin_a = flip_label ? ImSin(angle + IPI) : sin_a;
